...
# Open CSV and write header (append mode so repeated runs add rows)
LOG_DIR = "Time_data"
os.makedirs(LOG_DIR, exist_ok=True)
EVENTS_SEQ = os.path.join(LOG_DIR, "events_seq.csv")
if not os.path.exists(EVENTS_SEQ):
    with open(EVENTS_SEQ, "w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["chunk_id","record_start","record_end","infer_start","infer_end","label","score"])

def timestamp():
    return time.time()  # seconds since epoch, float

def main_sequential():
    output_wav_filename = FLAGS.wav
    ...
    while True:
        ...
        top_k = preds[0].argsort()[-1:][::-1]
        human_string = ""
        score = 0.0
        for node_id in top_k:
            human_string = load_labels(FLAGS.labels)[int(node_id)]
            score = float(preds[0,node_id])
            print(f'\tmodel predicted: {human_string} with score {score:.5f}')

        # append to CSV
        with open(EVENTS_SEQ, "a", newline="") as f:
            w = csv.writer(f)
            w.writerow([chunk_id, rec_start, rec_end, inf_start, inf_end, human_string, score])
